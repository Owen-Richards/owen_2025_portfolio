...
export default function ScrollProgress({
  sections = ['Home', 'Skills', 'About', 'Projects', 'Contact'],
}: ScrollProgressProps) {
  ...
  useEffect(() => {
    ...
    const updateScrollProgress = () => {
      ...
      const isContactInView = contactSection
        ? contactSection.getBoundingClientRect().top <= windowHeight * 0.5
        : false;
      ...
      const scrollYProgress =
        scrollableHeight > 0 ? currentScrollY / scrollableHeight : 1;
  ...
  return (
    ...
    >
      ...
      >
        {/* Background Circle */}
        <div
          className="h-16 w-16 rounded-full border shadow-lg backdrop-blur-md
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <motion.div className="text-center" style={{ color: colors.text }}>
            <motion.div className="text-xs font-bold" key={progressPercent}>
              {progressPercent}%
            </motion.div>
            <ChevronUp
              size={12}
              className="mx-auto opacity-60 transition-opacity group-hover:opacity-100"
            />
          </motion.div>
        </div>
      ...
      <div className="mt-2 flex justify-center gap-1">
        {sections.map((_, index) => (
          <motion.div
            ...
            style={{
              backgroundColor:
                index <= currentSection ? colors.progress : colors.progressBg,
            }}
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
