...
function FloatingGeometry({ 
  ...
  colors: { primary: string; secondary: string; accent: string; intensity: number };
}) {
...
function EnhancedLighting({ colors }: { colors: { intensity: number } }) {
  ...
  return (
    <>
      ...
      />
      <pointLight position={[-10, -10, -5]} color="#64748b" intensity={0.5 * colors.intensity} />
      <spotLight
        position={[0, 10, 0]}
        angle={0.15}
        penumbra={1}
...
function Scene({ colors }: { colors: { primary: string; secondary: string; accent: string; intensity: number } }) {
  ...
  return (
    <Suspense fallback={null}>
      ...
      <ParticleSystem count={1000} spread={25} speed={0.1} color={colors.primary} size={0.0008} />
      <ParticleSystem count={500} spread={30} speed={0.02} color={colors.secondary} size={0.0006} />
      ...
      {floatingGeometries.map((geo) => (
        <FloatingGeometry key={geo.key} position={geo.position} colors={colors} />
      ))}
...
export default function Hero3D({ isContinuous = false }: { isContinuous?: boolean }) {
  const { theme } = useTheme();
  
  return (
    <div className={isContinuous ? "fixed inset-0 -z-10" : "absolute inset-0 -z-10"}>
      <Canvas
        shadows
        dpr={[1, 2]}
